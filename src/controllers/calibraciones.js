...
export const closeCalibraciones = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await calibracion.update({ estado: 'CERRADO' });
    return res.status(200).json({
      success: true,
      message: `Calibración con ID ${id} cerrada exitosamente`,
    });
  } catch (error) {
    console.error('Error en closeCalibracione:', error);
    res.status(500).json({
      success: false,
      message: 'Error al cerrar la calibración',
      error: error.message,
    });
  }
};
...
export const delCalibraciones = async (req, res) => {
  try {
    const { ids } = req.body;
    ...
    res.status(200).json({
      success: true,
      message: `Calibraciónes eliminadas exitosamente`,
    });
  } catch (error) {
    console.error(error);
